// this file used to set any constants used in any screen , eg. shared pref consts,...
class SharedPrefsKeys {
  static const String PRAYER_TIME = "prayer time";
  static const String METHOD_NAME = "methodName";
  static const String SHOROUQ_OFFSET = "shorouqOffset";
  static const String MEDICATION_OFFSET = "medicationOffset";
  static const String DUHR_OFFSET = "duhrOffset";
  static const String ASR_OFFSET = "asrOffset";
  static const String MAGHRIB_OFFSET = "maghribOffset";
  static const String ISHA_OFFSET = "IshaOffset";
  static const String ADHAN_TIME_PREFERENCE = "adhanTimePreference";
  static const String ASR_TIME_PREFERENCE = "asrTimePreference";
  static const String DAYLIGHT_SAVING = "daylightSaving";
  static const String PRAYER_TIMES_SETTINGS = "prayerTimesSettings";
  static const String NOTIFICATION_SETTINGS = "notificationSettings";
  static const String USER_LOCATION = "userLocation";
  static const String APP_LANGUAGE = "appLanguage";
  static const String PRAYER_MAIN_RESPONSE = "prayerMainResponse";
  static const String CURRENT_THEME = "currentTheme";
  static const String ADHAN_NOTIFICATION_SETTINGS = "adhanNotificationSettings";
  static const String CALENDAR_SETTINGS = "calendarSettings";
  static const String PERMISSION_GRANTED = "permissionGranted";
  static const String ACTIVE_DAYLIGHT_MANULA = "activeDaylightManual";
  static const String MEDICATION_NOTIFICATION_SETTINGS = "Medication";


  static String MID_NIGHT = "mid_night";
  static const String ADHAN_FAJR_SLIDER_VALUE = "fajr_slider_value";
  static const String STATIC_LOCATION_KEY = "static_location_key";

  static String SERVICE_ENABLED = " service_enabled";
  static String SLIDER_INDEX = " slider_index";

  static String CALLED_API_DAY = "called_api_day";
}
